...
def downloadFile(ftp, remoteFile, localDir):
    ...
    if not os.path.isdir(localDir):
        print "Error: Local path [" + localDir + "] is NOT directory."
        return
    if not exists(ftp, remoteFile):
        print "Error: Remote path [" + remoteFile + "] does NOT exist."
        return
    if isDir(ftp, remoteFile):
...
def downloadDir(ftp, remoteDir, localDir):
    ...
    for f in L:
        if not isDir(ftp, f):
            downloadFile(ftp, f, newLocalDir)
        else:
            downloadDir(ftp, f, newLocalDir)
...
def nlist(ftp, path):
    ...
    ftp.retrlines("NLST " + path, callback)
    return L
...
def exists(ftp, path):
    try:
        ftp.size(path)
    except ftplib.error_perm: # 这里，可能path不存在，或者是文件
        try:
            ftp.cwd(path)
        except ftplib.error_perm: # 现在，path肯定就是不存在了
            return False
